...
import 'package:flutter_woo_getx_learn/pages/index.dart';
...
class ProductDetailsController extends GetxController
  ...
  _loadProduct() async {
    ...
    if (product?.images != null) {
      bannerItems = product!.images!
          .map((e) => KeyValueModel(key: '${e.id}', value: e.src ?? ''))
          .toList();
    }
    ...
    // 评论
    reviews = await ProductApi.reviews(ReviewReq(
      product: productId,
    ));
  ...
  Future<bool> _loadReviews(bool isRefresh) async {
    // 拉取数据
    // 评论
    var reviewsListTmp = await ProductApi.reviews(ReviewReq(
      // 刷新, 重置页数1
...
// 评论 下拉刷新
  ...
  void onAddCartTap() async {
    // 检查是否登录
    if (!await UserService.to.checkLogin()) {
      return;
    }
